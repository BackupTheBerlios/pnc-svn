...
public class Shape {
	...
	public Shape(int x, int y, int width, int height, Point[] points, Bitmap image) {
		this(x, y);
	...
	public Shape(int x, int y, int width, int height, Point[] points, int color) {
		this(x, y);
		blocks = new ArrayList<Block>();
		for (Point p : points) {
	...
	public void paint(Canvas g, int nx, int ny){
		for (Block p : blocks) {
			p.paint(g, nx, ny);
		}
	}
